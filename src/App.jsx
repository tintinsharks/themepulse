...
function Scan({ stocks, themes, onTickerClick, activeTicker, onVisibleTickers, liveThemeData: externalLiveData, onLiveThemeData, portfolio, watchlist, setPortfolio, setWatchlist, addToPortfolio, removeFromPortfolio, addToWatchlist, removeFromWatchlist, initialThemeFilter, onConsumeThemeFilter, stockMap, filters, themeHealth, momentumBurst, erSipLookup, headlinesMap, earningsMovers, pmErTickers, ahErTickers, pmTopMovers, ahTopMovers, historicalEarningsMovers, focusList, onAddFocus, onRemoveFocus, pipelineMeta, marketSession, aiQueue, setAiQueue, aiAnalyzed, setAiAnalyzed, authToken, crpLookup }) {
  ...
  const candidates = useMemo(() => {
    ...
    if (minRVol > 0) list = list.filter(s => { const rv = liveLookup[s.ticker]?.rel_volume ?? s.rel_volume; return rv != null && projectedRVol(rv) >= minRVol; });
  ...
  useEffect(() => {
    ...
    else if (scanTab === "short") tickers = shortCandidates.map(s => s.ticker);
    else return;
    ...
    <div style={{ flex: 1, minWidth: 0, overflowX: "auto", overflowY: "visible" }}>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
        ...
        <tbody>{shortCandidates.map(s => {
          const isActive = s.ticker === activeTicker;
          const isExpanded = expandedShort === s.ticker;
          const inPortfolio = portfolio?.includes(s.ticker);
          const inWatchlist = watchlist?.includes(s.ticker);
          const dig = gapperDigest[s.ticker] || {};
...
function EpisodicPivots({ stockMap, onTickerClick, activeTicker, onVisibleTickers, earningsMovers, pmErTickers, ahErTickers, headlinesMap, pmTopMovers, ahTopMovers, historicalEarningsMovers, focusList, onAddFocus, onRemoveFocus, liveThemeData, portfolio, watchlist, pipelineMeta, marketSession }) {
  ...
  const filteredHistoricalMovers = useMemo(() => {
    ...
    // Gap filter: ≥4% change AND ≥100K volume — session-aware with fallback
    if (gapOnly) {
      list = list.filter(m => {
                  ...
                  <div style={{ flex: "0 0 90px" }}>
                    ...
                    <input type="text" placeholder="3/14/26" value={tradeInput.entryDateDisplay ?? tradeInput.entryDate ?? ""} onChange={e => {
                      ...
                      if (m1) {
                        ...
                        updateTrade({ entryDate: `${yr}-${m1[1].padStart(2,"0")}-${m1[2].padStart(2,"0")}`, entryDateDisplay: v });
                      } else if (m2) {
                        updateTrade({ entryDate: `${m2[1]}-${m2[2].padStart(2,"0")}-${m2[3].padStart(2,"0")}`, entryDateDisplay: v });
                      } else {
                        updateTrade({ entryDateDisplay: v, entryDate: v });
                      }
                  ...
                  {tradeMode === "closed" && <>
                    <div style={{ flex: "0 0 90px" }}>
                      ...
                      <input type="text" placeholder="3/15/26" value={tradeInput.exitDateDisplay ?? tradeInput.exitDate ?? ""} onChange={e => {
                        ...
                        if (m1) {
                          const yr = m1[3].length === 2 ? "20" + m1[3] : m1[3];
                          updateTrade({ exitDate: `${yr}-${m1[1].padStart(2,"0")}-${m1[2].padStart(2,"0")}`, exitDateDisplay: v });
                        } else if (m2) {
                          updateTrade({ exitDate: `${m2[1]}-${m2[2].padStart(2,"0")}-${m2[3].padStart(2,"0")}`, exitDateDisplay: v });
                        } else {
                          updateTrade({ exitDateDisplay: v, exitDate: v });
                        }
                      }} style={{ ...inputStyle, padding: "4px 6px", fontSize: 11 }} />
                    </div>
                    <div style={{ flex: "0 0 80px" }}>
...
function LiveView({ stockMap, onTickerClick, activeTicker, onVisibleTickers, portfolio, setPortfolio, watchlist, setWatchlist, addToWatchlist, removeFromWatchlist, addToPortfolio, removeFromPortfolio, liveThemeData, homepage, erSipLookup, crpLookup }) {
  ...
  const wlThemeGroups = useMemo(() => {
    ...
    const built = Object.values(groups).map(g => {
      ...
      const avgAccel = accelDvTotal > 0 ? Math.round(accelWeighted / accelDvTotal * 10) / 10 : null;
      // $Vol-weighted avg closing range %
      let crDvTotal = 0, crWeighted = 0;
      allStocks.forEach(s => {
        const cr = s.close_range ?? liveLookup[s.ticker]?.close_range;
        if (cr == null) return;
        const dv = s.avg_dollar_vol_raw || stockMap[s.ticker]?.avg_dollar_vol_raw || 0;
        crDvTotal += dv;
        crWeighted += cr * dv;
...
function EarningsIntel({ earningsMovers = [], pmEarningsMovers = [], ahEarningsMovers = [], pmSipMovers = [], ahSipMovers = [], historicalEarningsMovers = [], stockMap = {}, liveThemeData, marketSession, onTickerClick, activeTicker }) {
  ...
  const calendarDays = useMemo(() => {
    ...
    const passFilter = (mv) => {
      ...
      if (avgVol == null) return false; // no avg volume data = skip
      if (isBioReit(mv.ticker, mv.company || mv.name)) return false;
      return true;
    };

    // Filter for non-earnings movers: avg volume > 500K, traded vol > 30K, bio/REIT
    const passSipFilter = (mv) => {
      const pipe = stockMap[mv.ticker];
      const avgVol = pipe?.avg_volume_raw ?? mv.avg_volume;
      if (avgVol != null && avgVol < 500_000) return false;
      const vol = mv.volume ?? 0;
      if (vol < 30_000) return false;
      if (isBioReit(mv.ticker, mv.company || mv.name)) return false;
      return true;
    };
    ...
    // ── Build set of tonight's AMC earnings reporters to exclude from today's PM bucket ──
    // PM earnings page includes last night's AMC reporters (gap reaction).
    // Only use ah_earnings_movers (tonight's live AH earnings page) — these reported AMC today.
    const todayStr = `${today.getFullYear()}-${String(today.getMonth()+1).padStart(2,"0")}-${String(today.getDate()).padStart(2,"0")}`;
    const tonightAhErSet = new Set();
    (ahEarningsMovers || []).forEach(mv => { if (mv.ticker) tonightAhErSet.add(mv.ticker); });

    // ── Today (day 0): PM movers, excluding tonight's AMC earnings reporters ──
  ...
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto" }}>
      ...
      {activeSection === "feed" && (() => {
        ...
        const tickerThemes = {};
        themes.forEach(t => (t.tickers || []).forEach(tk => {
          (tickerThemes[tk] = tickerThemes[tk] || []).push(t.name);
        }));

        // Combine current + historical, dedup by ticker (current wins)
        const seen = new Set();
        const allER = [];
          ...
          <div>
            ...
            {topThemes.length > 0 && (
              <div style={{ background: "#16161e", border: "1px solid #2a2a38", borderRadius: 8, padding: 14, marginBottom: 16 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#d4d4e0", marginBottom: 8 }}>Emerging Themes from Earnings</div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {topThemes.map(([name, count]) => (
                    <span key={name} style={{ fontSize: 10, padding: "4px 10px", borderRadius: 6, background: CYAN_BG, border: `1px solid ${CYAN}30`, color: CYAN, fontWeight: 600 }}>
                      {name} <span style={{ color: "#686878", marginLeft: 4 }}>{count}</span>
                    </span>
                  ))}
                </div>
              </div>
