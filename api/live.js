...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Congressional trades — short-circuit before Finviz login
    const congressTicker = (req.query.congress || "").trim().toUpperCase();
    if (congressTicker) {
      const fmpKey = process.env.FMP_API_KEY;
      if (!fmpKey) return res.status(200).json({ trades: [] });
      try {
        const fmpBase = "https://financialmodelingprep.com/stable";
        const [senate, house] = await Promise.all([
          fetch(`${fmpBase}/senate-trades?symbol=${encodeURIComponent(congressTicker)}&apikey=${fmpKey}`).then(r => r.ok ? r.json() : []).catch(() => []),
          fetch(`${fmpBase}/house-trades?symbol=${encodeURIComponent(congressTicker)}&apikey=${fmpKey}`).then(r => r.ok ? r.json() : []).catch(() => []),
        ]);
        const all = [
        ...
        res.setHeader("Cache-Control", "public, max-age=86400, stale-while-revalidate=43200");
        return res.status(200).json({ trades: all });
      } catch (e) {
        return res.status(200).json({ trades: [], error: e.message });
      }
    }
